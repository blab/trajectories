#!/usr/bin/env python3
"""
Train/test split for UShER-derived branches.tsv files.

This script reads branches_raw.tsv (which contains parent-child relationships)
and marks entire clades as "test" data similar to train_test_split.py.

The tree structure is built directly from the branches file, avoiding the need
to parse the full Newick tree (which may contain millions of unsubsampled nodes).
...
def main():
    ...
    parser.add_argument(
        "--tree",
        help="Input Newick tree file (ignored, kept for compatibility)"
    )
    parser.add_argument(
